...
import { circleStrokeWidth, circlePreviewStrokeWidth, lineStrokeWidth, linePreviewStrokeWidth, arrowShaftWidth, arrowPolygonPoints, requirePreviewAnnotations, validateTextAnnotation, type TextAnnotation } from "./editorAnnotations";
...
describe("circle stroke width", () => {
  it("keeps legacy data absent and defaults to three reference pixels", () => {
    const circle = { ...text, type: "circle" as const, rotation: 0 };
    expect(circleStrokeWidth()).toBe(3);
    expect(requirePreviewAnnotations(JSON.parse(serializeTimeline([], [], [circle])).annotations)).toEqual([circle]);
  });
  it.each([1, 2, 3, 6, 9])("scales and roundtrips %s reference pixels", strokeWidth => {
    for (const width of [1920, 960, 640]) expect(circlePreviewStrokeWidth(strokeWidth, width)).toBe(strokeWidth * width / 1920);
    const circle = { ...text, type: "circle" as const, rotation: 0, strokeWidth };
    expect(requirePreviewAnnotations(JSON.parse(serializeTimeline([], [], [circle])).annotations)).toEqual([circle]);
  });
  it.each([0, -1, 4, 10, NaN, Infinity])("rejects invalid circle width %s", strokeWidth => {
    expect(() => requirePreviewAnnotations([{ type: "circle", strokeWidth }])).toThrow("Invalid circle stroke width");
  });
});
